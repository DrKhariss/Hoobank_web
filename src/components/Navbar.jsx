    ...
    <nav className='w-full flex py-6 justify-between items-center navbar'>
      ...
      <ul className='list-none sm:flex hidden justify-end items-center flex-1'>
        {navLinks.map((nav, index) => (
          <li className=' transform transition-transform duration-300 hover:scale-110' key={nav.id}>
            <a
              key={nav.id} 
              href={`#${nav.id}`} 
        ...
        {navLinks.map((nav, index) => (
          <li>
            <a
              key={nav.id} 
              href={`#${nav.id}`} 
              className={`font-poppins font-normal cursor-pointer text-[16px] ${index === navLinks.length - 1 ? 'mr-0' : 'mb-10'}  text-white hover:text-secondary`}>
              {nav.title}
            </a>
          </li>
        ))}
